class AlarmClock extends Window {
  ...
  AlarmClock() {
    ...
    scroll = new Scroll("the duration of awakening is", 5, 20, "minutes", width*.5, height*.55);
    button = new Button("off", "on", width*0.5, height*0.8);
...
class LightPicker extends Window {
  ...
  LightPicker() {
    Scroll scroll = new Scroll("brightness: ", 0, 255, " / 255", width*0.5, height*0.57);
...
class Error extends Window {
  ...
  void draw() {
    ...
    text("Connection error\n" + 
         ...
         "* on your device, Wi-Fi is enabled"
         , 0, 0, width, height/2);
